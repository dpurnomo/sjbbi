	...
	<div class="container-fluid">
		<div class="row">
			<div class="col" style="margin-bottom:30px;">
				<div class="card">
					<div class="card-body">
						<h3 class="card-title"><i class="far fa-calendar-alt"></i> PERHIMPUNAN</h3>
  ...
  RIWAYAT
			...
			<div class="col">
				<div class="card">
					<div class="card-body">
						<h3 class="card-title">
							<i class="fab fa-elementor"></i> Kebutuhan Setempat
						</h3>

							<div class="list-group-flush">
							<?php include_once('includes/dbconfig.php');
$sql = 'SELECT id, type, date, tp_kebsetempat_is, tp_bagian1, tp_kebsetempat FROM Meetings WHERE tp_kebsetempat_is=1 AND date >= CURDATE() ORDER BY date';
		$result = $conn->query($sql);
		...
		if ($result->num_rows > 0) {
		    ...
		    while($row = $result->fetch_assoc()) {
					...
					echo '<a href="./meeting_show.php?id='.$row['id'].'&p='.$page.'" class="list-group-item list-group-item-action flex-column align-items-start">';
					echo '<div class="d-flex w-100 justify-content-between">';
					echo '<div class="font-weight-bold"><h4>'.date_format($date, 'M Y').'</h4></div>';
					echo '<div class="text-muted"><small>'.date_format($date, 'D, d/m/y').'</small></div></div>';
	...
	//	        echo "<tr><td><a href=''>".$row["type"]."</a></td><td>".date_format($date, 'D, d/m/y')."</td></tr>";
		    }
			echo "</a>";
//		    echo "</table></center></div>";
		} else {
